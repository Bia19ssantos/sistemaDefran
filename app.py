...
with aba3:
    ...
    if os.path.exists(caminho_pdf):
        ...
        st.markdown(f'<a href="data:application/pdf;base64,{b64_pdf}" target="_blank"><button style="width:100%; background-color:#007bff; color:white; padding:12px; border:none; border-radius:5px; font-weight:bold;">📄 Abrir PDF em Nova Aba</button></a>', unsafe_allow_html=True)

# --- ABA 4: CADASTRO DE ORÇAMENTO ---
with aba4:
    st.header("📋 Cadastro e Geração de Orçamento")
    
    tipo_orcamento = st.selectbox("Modelo de Orçamento:", ["Produtos", "Lingas"])
    
    col_c1, col_c2, col_c3 = st.columns(3)
    num_orc = col_c1.text_input("Nº da Proposta", value="373/26")
    data_orc = col_c2.text_input("Data", value="19 de agosto de 2026")
    cliente_orc = col_c3.text_input("Cliente", value="SIVA")
    
    col_c4, col_c5, col_c6 = st.columns(3)
    cidade_orc = col_c4.text_input("Cidade", value="ITAQUAQUECETUBA")
    estado_orc = col_c5.text_input("Estado", value="SP")
    tel_orc = col_c6.text_input("Telefone", value="(11) 4646-4646")

    col_c7, col_c8 = st.columns(2)
    contato_orc = col_c7.text_input("Contato", value="MARCELO")
    email_orc = col_c8.text_input("E-mail", value="compras@siva.com.br")
    vendedor_orc = st.text_input("Vendedor(a)", value="BEATRIZ")

    st.markdown("---")
    st.subheader("Adicionar Itens via Busca Automática")
    
    # Campo de busca da referência
    ref_digitada = st.selectbox("Digite ou selecione a Referência do Produto:", [""] + list(base_produtos.keys()))
    
    # Preenchimento automático com base no dicionário
    ...
    with st.form("form_item_orc", clear_on_submit=True):
        ...
        item_ref = col_i1.text_input("Referência Exata", value=dados_encontrados.get("referencia", ""))
        item_qtd = col_i2.number_input("Quantidade / Metragem", min_value=0.01, value=1.00, step=0.01)
        ...
        if add_item_btn and item_ref:
            # Regra de unidade automática: 'M' se for corrente, senão 'PÇ'
            texto_verificacao = (item_ref + " " + item_desc).lower()
            unidade = "M" if "corrente" in texto_verificacao else "PÇ"
    ...
    if "itens_orcamento" in st.session_state and st.session_state.itens_orcamento:
        ...
        st.dataframe(df_itens[['referencia', 'quantidade', 'unidade', 'unitario', 'total']], use_container_width=True)
        ...
        def gerar_pdf_defran(tipo, num, data, cliente, cidade, estado, tel, contato, email, vendedor, itens):
            buffer = BytesIO()
            ...
            elementos.append(Paragraph(f"<b>PROPOSTA COMERCIAL ({tipo.upper()})</b>", estilo_titulo))
        ...
        if st.button("📄 Gerar e Baixar Proposta Comercial em PDF"):
            pdf_buffer = gerar_pdf_defran(
                tipo_orcamento, num_orc, data_orc, cliente_orc, 
